...
func GetPkgImportPath(name string)(string ,error ){
	gosrc := GetGOPATH() + "/src/"
	gosrc = strings.Replace(gosrc, "\\", "/", -1)
	pwd, err := os.Getwd()
	if err != nil {
	...
	pwd = strings.Replace(pwd, "\\", "/", -1)
	projectPath := strings.Replace(pwd, gosrc, "", 1)
	pkgPath := fmt.Sprintf(viper.GetString("gk_pkg_path_format"), ToLowerSnakeCase(name))
	pkgPath = strings.Replace(pkgPath, "\\", "/", -1)
	pkgImport := projectPath + "/" + pkgPath
