...
var opImport = "github.com/opentracing/opentracing-go"
...
func NewGenerateService(name, transport string, sMiddleware, gorillaMux, eMiddleware bool, methods []string) Gen {
	pkgImport, _ = utils.GetPkgImportPath(name)
	i := &GenerateService{
		name:          name,
...
func (g *GenerateService) Generate() (err error) {
	...
	g.generateNewMethod()
	svcSrc = g.srcFile.GoString() + "\n" + changePkgAndImp(svcSrc)
...
func (g *GenerateService) generateServiceMethods() {
	...
	for _, m := range g.serviceInterface.Methods {
		...
		body := []jen.Code{
			jen.Var().Id("parentCtx").Qual(opImport, "SpanContext"),
			jen.Var().Id("span").Qual(opImport, "Span"),
			jen.If(
				jen.Id("parentSpan").Op(":=").Qual(
					opImport, "SpanFromContext").Call(jen.Id("ctx")).Op(";").Id("parentSpan").Op("!=").Id("nil"),
			).Block(
				jen.Id("parentCtx").Op("=").Id("parentSpan").Dot("Context").Call(),
				jen.Id("span").Op("=").Id(stp).Dot("tracer").Dot("StartSpan").Call(
					jen.Lit(m.Name),
					jen.Qual(opImport, "ChildOf").Call(jen.Id("parentCtx")),
				),
				jen.Id("defer").Id("span").Dot("Finish").Call(),
				jen.Id("ctx").Op("=").Qual(opImport, "ContextWithSpan").Call(
					jen.Id("ctx"),
					jen.Id("span"),
				),
			),
...
func (g *GenerateService) generateServiceStruct() {
	...
	g.code.appendStruct(
		g.serviceStructName,
		jen.Id("tracer").Qual(opImport, "Tracer"),
...
func (g *generateCmdBase) Generate() (err error) {
	...
	if g.generateEndpointDefaultsMiddleware {
		...
		for _, m := range g.serviceInterface.Methods {
			...
			mdw[m.Name] = append(
				mdw[m.Name],
				jen.Id("opentracing").Dot("TraceServer").Call(
					jen.Id("tracer"),
					jen.Lit(m.Name),
				),
			)
...
func (g *generateCmd) generateCmdMain() error {
	mainDest := fmt.Sprintf(viper.GetString("gk_project_path_format"), g.name)
